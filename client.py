...
class Client(Socket):
    def __init__(self):
        ...
        if len(os.listdir('C:\\Users\\azali\\PycharmProjects\\pythonProject\\friend_id')) != 0:
            pass
        else:
            print("Поместите идентификатор собеседника в friend_id")

        # Проверка создан ли личный идентификатор
        if not os.path.exists('C:\\Users\\azali\\PycharmProjects\\pythonProject\\private'):
            print("Также необходимо сгенерировать свой идентификатор")

        if len(os.listdir('C:\\Users\\azali\\PycharmProjects\\pythonProject\\friend_id')) != 0 and os.path.exists('C:\\Users\\azali\\PycharmProjects\\pythonProject\\private'):
            # Подгрузка данных текущего клиента
            with shelve.open('C:\\Users\\azali\\PycharmProjects\\pythonProject\\private\\private') as file:
            ...
            if not os.path.exists('C:\\Users\\azali\\PycharmProjects\\pythonProject\\private'):
                ...
                while True:
                    if msvcrt.kbhit():  # Нажата ли клавиша?
                        ...
                        if key == 27:  # если Enter:
                            ...
                            with shelve.open('pubkey') as file:
                                file['pubkey'] = pubkey
                            os.makedirs('C:\\Users\\azali\\PycharmProjects\\pythonProject\\private')
                            with shelve.open('C:\\Users\\azali\\PycharmProjects\\pythonProject\\private\\private') as file:
                                file['pubkey'] = pubkey
                    ...
                    if msvcrt.kbhit():  # Нажата ли клавиша?
                        key = ord(msvcrt.getch())  # Какая клавиша нажата?
                        print(key)
                        if key == 9 and len(os.listdir('C:\\Users\\azali\\PycharmProjects\\pythonProject\\friend_id')) != 0:
                            print("Op")
                            try:
                                # Подгрузка данных собеседника
                                with shelve.open('C:\\Users\\azali\\PycharmProjects\\pythonProject\\friend_id\\pubkey') as file:
                                    self.friend_public_key = file['pubkey']
                                    print(self.friend_public_key)
                            except:
